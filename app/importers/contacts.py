...
def do_import(user: str, filename: str, vcf_bytes: bytes) -> dict:
    text = _decode(vcf_bytes)
    written = 0
    with radicale_store.storage_lock():
        coll = radicale_store.ensure_collection(
            user, _CONTACTS_COLLECTION, tag="VADDRESSBOOK", displayname="Kontakte"
        )
