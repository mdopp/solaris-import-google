...
note to an Obsidian Markdown file with YAML frontmatter and write it under
``notes/users/<user>/Google Keep/``; attachments are copied into an
``attachments/`` subfolder and embedded with Obsidian ``![[...]]`` links.
...
def do_import(user: str, files: list[tuple[str, bytes]]) -> dict:
    file_map = expand_uploads(files)
    target = identity.keep_target_dir(user)
    ...
    return {
        ...
        "target": f"notes/users/{user}/{identity.config.KEEP_SUBDIR}",
    }
